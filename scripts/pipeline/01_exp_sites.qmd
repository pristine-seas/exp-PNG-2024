...
core_site_fields <- c("exp_id", "leg", "survey_type", "ps_site_id", "region", "subregion", "locality",
                      "date", "time", "latitude", "longitude", "lead", "notes")

# Useful functions 

## Assign depth strata 
assign_depth_stratum <- function(depth_m, 
                                 shallow_threshold = 13,
...
This script processes and uploads all **site-level metadata** for the **`r exp_id`** expedition to the Pristine Seas Database in BigQuery. It consolidates fieldbook records across all methods into a single, unified pipeline for QAQC, mapping, and upload.

The goals of this script are to:

1.  **Import fieldbook data** for all applicable methods. 
2.  **Standardize site metadata** according to the Pristine Seas core site schema.
3.  **Perform QAQC checks** across methods to catch early issues, such as:
    -   Duplicate or malformed site IDs
    -   Missing or invalid coordinates
    -   Inconsistent location, habitat, or exposure values
4.  **Visualize all sites** in an interactive map to confirm spatial accuracy and coverage.
5.  **Summarize** sampling effort by method and locations.
5.  **Upload cleaned site tables** to the appropriate method-specific datasets in BigQuery.

This centralized approach enables early validation, visualization, and reporting of spatial data across methods. While downstream datasets (e.g., BRUVS video analysis) may take months to process, site metadata can be uploaded immediately after fieldwork using fieldbooks—providing a solid foundation for future data integration and analysis.

## QAQC model

A crucial early step in building the Pristine Seas Database is to run quality checks on our fieldbooks before uploading them to BigQuery. These QA/QC checks help ensure that the data is complete, consistent, and ready for integration. The standard validation checks include:

  - **No duplicated site IDs**: Each ps_site_id must be unique.
  - **Invalid habitat values**: All values in the habitat column must match the controlled vocabulary:
"fore reef", "back reef", "fringing reef", "lagoon", "patch reef".
  - **Invalid exposure values**: All values in the exposure column must match the controlled vocabulary:
"leeward", "windward", "lagoon".
  - **Missing or invalid coordinates**: All sites must have non-missing latitude and longitude values, and they must fall within valid ranges (lat: −40 to 40, lon: −180 to 180).
  - **Missing or invalid date**: All sites must have non-missing latitude and longitude values, and they must fall within valid ranges (lat: −40 to 40, lon: −180 to 180).
  -	**Visualize missing data**: Generate a missingness map to quickly inspect null values across the table.
  
## Surveys

This script processes site metadata from all scientific methods conducted during the expedition and harmonizes them according to the Pristine Seas **core site schema**. The following sections read the fieldbooks, apply QAQC and standardization procedures, and output cleaned site tables ready for upload.

::: {.aside #core-site-fields}
**Core Site Fields:**  
`exp_id`, `leg`, `survey_type`, `ps_site_id`, `region`, `subregion`, `locality`, `date`, `time`, `latitude`, `longitude`, `lead`.

These form the minimal metadata required to geolocate and contextualize every sampling site.
:::
