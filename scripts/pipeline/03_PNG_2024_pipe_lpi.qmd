...
lpi_stations <- lpi_sections |> 
  ...
  select(ps_station_id, ps_site_id, exp_id,              
         ...
         divers = diver, depth_m, depth_strata,          
...
lpi_contacts <- lpi_contacts |> 
  left_join(lpi_stations |> 
              select(ps_station_id, exp_id, diver = divers, depth_m)) |> 
  left_join(lpi_sections |> 
              select(section_id, transect_label, transect_section = section)) |> 
  group_by(ps_station_id, exp_id, diver, depth_m, transect_label, transect_section, group, morphotaxon, field_name, section_id) |> 
...
coral_contacts <- coral_contacts |> 
  ...
  group_by(section_id, ps_station_id, ps_site_id, exp_id, diver,  depth_m,depth_strata, transect_label, survey_dist_m, transect_section, field_name, morphotaxon) |> 
...
taxa_proportions <- coral_contacts |> 
  ...
  ungroup() |> 
  filter(contacts > 0) |> 
  select(ps_station_id, section_id, field_name, morphotaxon, prop_contacts)
...
match_lpi_only <- section_alignment |> 
  ...
  mutate(field_name = "Scleractinia",
         morphotaxon =  "Scleractinia",
...
divers_per_station <- complete_lpi_data |> 
  ungroup() |> 
  group_by(ps_station_id) |> 
  summarize(divers = paste0(unique(diver), collapse = " | ")) |> 
  ungroup()

lpi_stations <- lpi_stations |> 
  select(-divers, -lpi_pct_coral, -lpi_coral_pts) |> 
...
clean_lpi <- lpi_observations |> 
  ungroup() |> 
  select(-field_name, -functional_group) |> 
...
clean_lpi <- clean_lpi |> 
  mutate(functional_group = case_when(morphotaxon %in% c("Barren", "Sediment", "Coral rubble") ~ "sediment|rubble|barren", 
                                      ...
                                      functional_group == "Hard coral" ~ "hard_coral",
...
cover_by_station_and_group <- cover_by_station_and_taxa |> 
  mutate(functional_group = case_when(functional_group %in% c("bryozoans","worms","echinoderms", "molluscs", "hydrozoans", "ascidians", "other_cnidarians") ~ "other",
                                      functional_group %in% c("hard_coral_dead") ~ "sediment|barren|rubble",
                                      TRUE ~ functional_group)) |> 
  ...
  rename(pct_coral = pct_hard_coral,
         pct_rubble = pct_sediment_barren_rubble,
...
cover_by_station_and_taxa |> 
  mutate(functional_group = case_when(functional_group %in% c("bryozoans","worms","echinoderms", "molluscs", "hydrozoans", "ascidians", "other_cnidarians") ~ "other",
                                      ...
                                      TRUE ~ functional_group)) |> 
  mutate(functional_group = fct_relevel(functional_group, 
                                        grps_order)) |> 
  group_by(ps_site_id, depth_strata, functional_group) |> 
  summarize(pct_cover = round(sum(pct_cover),2),
            .groups = "drop") |> 
  ...
  facet_wrap(~depth_strata, ncol = 1)+
