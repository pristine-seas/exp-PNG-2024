...
uvs_sites <- read_xlsx(file.path(exp_path, "data/primary/raw/_PNG-2024-uvs-metadata.xlsx")) |> 
  ...
  mutate(exp_id    = exp_id,
         ...
         in_mpa    = if_else(in_mpa == "yes", TRUE, FALSE),
         notes     = str_to_lower(notes),
         # Standardize site IDs to match database format (3-digit padding)
         ps_site_id = str_replace_all(ps_site_id, "SLB_UVS", "SLB_2024_uvs"),
...
plot_uvs_sites_map(region = "Marau", subregion = NULL,
                   save = TRUE, path = file.path(exp_path, "figures/site_maps/", "uvs_Marau.pdf"))

plot_uvs_sites_map(region = "Three Sisters", subregion = NULL,
                   save = TRUE, path = file.path(exp_path, "figures/site_maps/", "uvs_Three_sisters.pdf"))

plot_uvs_sites_map(region = "Temotu", subregion = "Tinakula",
                   save = TRUE, path = file.path(exp_path, "figures/site_maps/", "uvs_Tinakula.pdf"))

plot_uvs_sites_map(region = "Temotu", subregion = "Lata",
                   save = TRUE, path = file.path(exp_path, "figures/site_maps/", "uvs_Lata.pdf"))

plot_uvs_sites_map(region = "Temotu", subregion = "Reef Islands",
                   save = TRUE, path = file.path(exp_path, "figures/site_maps/", "uvs_Reef_Islands.pdf"))

plot_uvs_sites_map(region = "Temotu", subregion = "Nukapu",
                   save = TRUE, path = file.path(exp_path, "figures/site_maps/", "uvs_Nukapu.pdf"))

plot_uvs_sites_map(region = "Temotu", subregion = "Duff Islands",
                   save = TRUE, path = file.path(exp_path, "figures/site_maps/", "uvs_Duff_Islands.pdf"))
```
...
write_csv(uvs_sites, file.path(exp_path, "data/primary/output",  "SLB_2024_uvs_sites_clean.csv"))
```
