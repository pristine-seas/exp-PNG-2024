...
invert_avg_by_site <- invert2 %>% 
  ...
  dplyr::summarise(count = sum(count), ind_m2 = sum(count)/100)


invert_avg_by_station <- invert2 %>% 
  filter(transect != "Off") %>%
  group_by(ps_station_id, common_name, taxa) %>%
  dplyr::summarise(count = sum(count), ind_m2 = sum(count)/50)


### Teasing out the more important inverts

resource_invert_avg_by_site<-invert_avg_by_site %>%
...
save(invert_avg_by_site, invert_avg_by_station, resource_invert_avg_by_site,invert_taxa_table, raw_cleaned_inverts,  file = "~/Google Drive/My Drive/Pristine Seas/SCIENCE/expeditions/PNG-2024/data/primary/processed/invert/PNG_2024_uvs_invert.RData")
