...
# Need to get number of transect to calculate density here
transects_leg2<-stations$fish %>%
  distinct(ps_site_id, n_transects)%>%
  group_by(ps_site_id) %>%
  dplyr::summarise(n_transects = sum(n_transects)) %>%
   mutate(area_m2 = 50*n_transects) 
...
invert_leg1_avg_by_site <- invert2 %>% 
  ...
  group_by(ps_site_id , common_name, taxon_valid_name) %>%
    dplyr::summarise(n_individuals = n())%>%
      left_join(.,leg1_transect_area, by  = "ps_site_id") %>%
  as.data.frame()
...
resource_invert_leg1_avg_by_site<-invert_leg1_avg_by_site %>%
  filter(common_name == "Giant clams" | common_name == "Sea cucumber")



# Jen said her transects were 25 x 2 so each is 50m
resource_invert_leg2_avg_by_site<-invert2 %>%
    filter(transect != "Off", leg == "leg 2") %>%
filter(common_name == "Giant clams" | common_name == "Sea cucumber",count != 0)%>%
  group_by(ps_site_id, common_name, taxon_valid_name) %>%
   dplyr::summarise(n_individuals = n()) %>%
    left_join(.,site_transect_fish, by = "ps_site_id") %>%
  mutate(area_m2 = 50*n_transects) %>%
     group_by(ps_site_id, common_name, taxon_valid_name,n_individuals, area_m2) %>%
  dplyr::summarise(ind_m2 = round(n_individuals/area_m2,2))

resource_invert_leg2_avg_by_site$n_transects<-NULL


resource_inverts_avg_by_site<-rbind(resource_invert_leg1_avg_by_site, resource_invert_leg2_avg_by_site)


raw_cleaned_inverts<-invert2


save(invert_leg1_avg_by_site, invert_leg1_avg_by_station, resource_inverts_avg_by_site,invert_taxa_table, raw_cleaned_inverts,  file = "~/Google Drive/My Drive/Pristine Seas/SCIENCE/expeditions/PNG-2024/data/primary/processed/invert/PNG_2024_uvs_invert.RData")
# 
 save(invert_leg1_avg_by_site, invert_leg1_avg_by_station, resource_inverts_avg_by_site,invert_taxa_table, raw_cleaned_inverts,  file = "PNG_2024_uvs_invert.RData")
