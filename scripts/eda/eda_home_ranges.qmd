...
custom_polygon_1 <- custom_polygon |> 
  st_union()

tmp <- custom_polygon_1 |> 
  st_difference(png_12nm) 

plot(tmp)

tmp |> 
  st_write(file.path(exp_path, "data/primary/processed/western-islands/proposal_v1_cut.gpkg"), 
           delete_dsn = TRUE)

png_12nm |> 
  st_difference(png_land_western) |> 
  st_difference(png_islands) |> 
  st_write(file.path(exp_path, "data/primary/processed/western-islands/png_12nm.gpkg"), 
           delete_dsn = TRUE)
```
...
tmp_df |>
  ...
  geom_sf(data = st_transform(tmp, 4326), fill = "transparent", color = "#3EFF8B", linewidth = .5)+
  geom_sf(data = st_transform(buffer_redfoot_booby, 4326), fill = "transparent", color = "pink", linewidth = .5)+
  geom_sf(data = st_transform(png_12nm, 4326), fill = "transparent", color = "lightblue", linewidth = .5)+
  geom_sf(data = st_transform(provincial_waters_only, 4326), fill = "transparent", color = "lightblue", linewidth = .5)+
  ...
  labs(title = "Drifting Longline Fishing Effort in Papua New Guinea EEZ",
       ...
       caption = "Data: AIS from Global Fishing Watch 2016-2024 | Black: PNG EEZ Boundary | Green: large MPA | Pink: 45nm MPA | Blue: Provincial Waters")
...
tmp_df |>
  ...
  labs(title = "Tuna Purse Seine Fishing Effort in Papua New Guinea EEZ",
       subtitle = "Western Islands Conservation Opportunities",
       caption = "Data: AIS from Global Fishing Watch 2016-2024 | Black: PNG EEZ Boundary | Green: large MPA | Pink: 45nm Buffer | Blue: Provincial Waters (12 n.m.)")
