...
PNG_benthos_health$algae<- PNG_benthos_health$erect.algae+PNG_benthos_health$turf

PNG_benthos_health2<-PNG_benthos_health %>%
  select(ps_station_id, cca, hard.coral...live, algae, cyanobacteria) %>% 
...
poor_region_sites<-PNG_benthos_health2 %>%
  ...
  rename(other = "FALSE", poor = "TRUE") %>%
  data_frame() %>%
 mutate(total = other + poor)%>%
  mutate(poor_per = (poor/total)*100)
...
good_region_sites<-PNG_benthos_health2 %>%
  ...
  tally() %>%
  pivot_wider(id_cols = region, names_from = rank, values_from = n) %>%
  rename(other = "FALSE", good = "TRUE") %>%
  data_frame() %>%
 mutate(total = other + good)%>%
  mutate(good_per = (good/total)*100)
