...
waffle_df <- stations |> 
  filter(subregion == "Tingwon") |> 
  group_by(subregion) |> 
  summarize(n_stations = n_distinct(ps_station_id),
            biomass_gm2 = mean(biomass_gm2),
            across(contains("pct"), mean)) |> 
  mutate(across(contains("pct"), ~.*biomass_gm2/100)) |> 
  select(-biomass_gm2) |> 
  pivot_longer(cols = -c(subregion, n_stations),
               names_to = c(".value", "trophic_group"),
               names_pattern = "([a-z]+)_(.*)") |> 
  rename(biomass_gm2 = pct) |> 
...
waffle_df |>
  ...
  ungroup() |>
  mutate(trophic_group = fct_reorder(trophic_group, biomass_gm2)) |>  # Ascending order
  mutate(trophic_group = fct_rev(trophic_group)) |>  # Then reverse it
  ggplot(aes(fill = trophic_group, values = biomass_gm2))+
...
waffle_df |>
  ...
  summarise(biomass_gm2 = sum(biomass_gm2)) |>
  ungroup() |>
  arrange(desc(biomass_gm2)) |>  # Start with largest first
  mutate(# Create cumulative positions to force order
    cumsum_biomass = cumsum(biomass_gm2),
    lag_cumsum = lag(cumsum_biomass, default = 0),
    # Create new values that will force the right fill order
    new_values = case_when(trophic_group == "shark" ~ max(cumsum_biomass) + 1,  # Force shark to end
                           TRUE ~ biomass_gm2
    )
  ) |>
  ggplot(aes(fill = reorder(trophic_group, -new_values),  # Reverse order by new values
             values = biomass_gm2)) +  # But use original values for size
  geom_waffle(n_rows = 10,
              size = .2, 
              flip = F,
              colour = "white",
