...
```{r}
trophic_colors <- c("herbivore/detritivore" = "#009E73",  # Green for herbivores/detritivores
                    "lower_carnivore"       = "#E69F00",  # Amber for lower carnivores
                    "planktivore"           = "#56B4E9",  # Sky blue for planktivores
                    "shark"                 = "#D55E00",  # Orange/red for sharks
                    "top_predator"          = "#CC79A7" )  # Pink/purple for top predators
...
create_trophic_biomass_plot <- function(region_name) {
  ...
  p <- region_data |> 
    ...
    labs(x = "", y = "Average biomass (g/m²)", fill = "",
         title = paste0("Fish Biomass by Trophic Group and Subregion - ", region_name),
         subtitle = "Stacked bar plot showing mean biomass contributions by trophic group") +
...
murat_trophic_plot <- create_trophic_biomass_plot(region_name = "Lovongai")

# Get all unique regions in the dataset
all_regions <- unique(biomass_by_station_and_trophic$region)

# Create a filename-safe version of region names (replacing spaces with underscores)
safe_region_names <- gsub(" ", "_", all_regions)

# Iterate through all regions and create plots
for (i in seq_along(all_regions)) {
  # Get the current region name and its safe version for filenames
  region_name <- all_regions[i]
  safe_name <- safe_region_names[i]
  
  # Create the plot
  p <- create_trophic_biomass_plot(region_name)
  
  # Generate the file path with safe filename
  file_path <- file.path(exp_path, paste0("figures/uvs_fish_biomass_by_trophic_by_subregion_", 
                                         safe_name, ".pdf"))
  ...
  message(paste0("Created plot for region: ", region_name))
}
```
