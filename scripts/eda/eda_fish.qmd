...
trophic_colors <- c(
  "Shark" = "#000000",                # Black
  "Other Top Predator" = "#8B0000",         # Dark red
...
plot_data <- trophic_map_data |>
  ...
  mutate(trophic_group = case_when(trophic_group == "top_predator" ~ "Other Top Predator",
         ...
         trophic_group = factor(trophic_group, 
                                levels = c("Shark", "Other Top Predator","Lower Carnivore", "Herbivore/Detritivore", "Planktivore")),
...
plot_data |> 
  ...
  summarize(biomass_value = median(biomass_value, na.rm = TRUE)) |> 
  ggplot(aes(x = region, 
             y = biomass_value, 
             fill = trophic_group)) +
  ...
  labs(title = "Fish Biomass by Trophic Group",
       subtitle = "Comparison across regions",
       x = "",
       y = "Median % Biomass") +
  theme_minimal(base_size = 11) +
  theme(plot.title = element_text(size = 14, face = "bold", margin = margin(b = 5)),
...
```{r}
rmi_refs <- read_csv(file.path(ps_paths$expeditions, "RMI-2023/data/primary/output/fish-surveys/fish_surveys_summary_by_trophic.csv")) |> 
  mutate(across(where(is.character), str_to_title))

rmi_refs <- rmi_refs |> 
  group_by(location) |> 
  summarise(pct_predators = sum(pct_biomass[trophic %in% c("Sharks", "Other Top Predators")])) |> 
  mutate(exp_id = "RMI-2023",
         region = str_extract(location, "^[^-]+")) |> 
  group_by(exp_id, region) |> 
  summarize(mean_pct_predators = mean(pct_predators, na.rm = TRUE),
             pct_other = 100 - (mean_pct_predators))

palau_refs <- read_csv(file.path(ps_paths$expeditions, "PLW-2024/data/primary/processed/PLW_2024_fish_surveys_biomass_by_taxa_and_station.csv")) 

palau_sites <- read_csv(file.path(ps_paths$expeditions, "PLW-2024/data/primary/processed/PLW_2024_fish_surveys_metadata.csv")) 
palau_trophs <- read_csv(file.path(ps_paths$expeditions, "PLW-2024/data/primary/processed/PLW_2024_fish_surveys_taxa_summary.csv")) |> 
  select(taxon_valid_name, trophic)

palau_refs <- palau_refs |> 
  left_join(palau_trophs) |> 
  left_join(palau_sites |> distinct(ps_site_id, location)) |> 
  group_by(location, ps_site_id, depth_strata, trophic) |>
  summarize(biomass_gm2 = sum(gr_m2, na.rm = TRUE),
            .groups = "drop") |> 
  pivot_wider(names_from = trophic, values_from = biomass_gm2, values_fill = 0, names_prefix = "grm2_") |> 
  pivot_longer(cols = starts_with("grm2_"),
               names_to = "trophic_group",
               values_to = "biomass_gm2") |> 
  group_by(location, ps_site_id, depth_strata) |> 
  mutate(pct_biomass = round(100*biomass_gm2/sum(biomass_gm2))) |> 
  ungroup() |> 
  group_by(location, ps_site_id, trophic_group) |> 
  summarize(biomass_gm2 = mean(biomass_gm2, na.rm = TRUE),
            pct_biomass = mean(pct_biomass, na.rm = TRUE)) |> 
  group_by(location, trophic_group) |> 
  summarize(biomass_gm2 = mean(biomass_gm2, na.rm = TRUE),
            pct_biomass = mean(pct_biomass, na.rm = TRUE),
            .groups = "drop") |> 
  mutate(exp_id = "PLW_2024",
         region = location) |> 
  group_by(exp_id, region) |> 
  summarize(mean_pct_predators = round(sum(pct_biomass[trophic_group %in% c("grm2_Top-predator sharks", "grm2_Top-predator")], na.rm = TRUE))) |> 
mutate(pct_other = 100 - mean_pct_predators)
...
data <- stations |> 
  group_by(exp_id, region) |> 
  summarize(biomass_gm2 = mean(biomass_gm2),
            pct_shark = mean(pct_shark),
            pct_top_predator = mean(pct_top_predator)) |> 
  mutate(mean_pct_predators = pct_shark + pct_top_predator,
        pct_other = 100 - (mean_pct_predators)) |> 
  bind_rows(rmi_refs) |> 
  bind_rows(palau_refs) |> 
  ungroup()


data |> 
  add_row(exp_id = "Northern Line Islands", region = "Kingman", mean_pct_predators = 85, pct_other = 15) |> 
  add_row(exp_id = "Northern Line Islands", region = "Palmyra", mean_pct_predators = 65, pct_other = 100 - 65) |>
  add_row(exp_id = "Northern Line Islands", region = "Jarvis", mean_pct_predators = 74, pct_other = 100 - 74) |>
  add_row(exp_id = "Northern Line Islands", region = "Baker", mean_pct_predators = 46, pct_other = 100 - 46) |>
  add_row(exp_id = "Northern Line Islands", region = "Howland", mean_pct_predators = 54, pct_other = 100 - 54) |>
  add_row(exp_id = "NWHI", region = "NWHI", mean_pct_predators = 54, pct_other = 100 - 54) |>
  pivot_longer(cols = c("mean_pct_predators", "pct_other"),
               names_to = "trophic_group",
               values_to = "percent") |> 
  mutate(label = paste(exp_id, region)) |> 
  ggplot()+
  geom_col(aes(x = fct_reorder(label,percent, sum), y = percent, fill = trophic_group))+
  coord_flip()
```
...
# ---- pull & filter (adjust to your schema) ------------------------------
stations <- tbl(bq_connection, "uvs.blt_stations") %>%
  filter(!is.na(exp_id)) %>%
  # Example "recent" filter: last 10 years; replace with your criterion
  # filter(year >= 2015) %>%
  # Keep 10 m and 20 m strata only (use your actual field/labels)
  # filter(depth_strata %in% c("10m","20m")) %>%
  collect()
...
png_summ <- stations %>%
  group_by(exp_id, region) %>%
  summarise(
    biomass_gm2       = mean(biomass_gm2, na.rm = TRUE),
    pct_shark         = mean(pct_shark, na.rm = TRUE),
    pct_top_predator  = mean(pct_top_predator, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(
    mean_pct_predators = pct_shark + pct_top_predator,
    pct_other          = 100 - mean_pct_predators
  )

# Ensure reference tables have the same columns (fill missing with NA)
std_cols <- c("exp_id","region","biomass_gm2","pct_shark","pct_top_predator",
              "mean_pct_predators","pct_other")

rmi_std   <- rmi_refs   %>% mutate(biomass_gm2 = NA_real_, pct_shark = NA_real_,
                                   pct_top_predator = NA_real_) %>% select(all_of(std_cols))
palau_std <- palau_refs %>% mutate(biomass_gm2 = NA_real_, pct_shark = NA_real_,
                                   pct_top_predator = NA_real_) %>% select(all_of(std_cols))

data <- png_summ %>%
  bind_rows(rmi_std, palau_std) %>%
  # Add explicit named references
  add_row(exp_id = "Northern Line Islands", region = "Kingman",  mean_pct_predators = 85, pct_other = 15) %>%
  add_row(exp_id = "Northern Line Islands", region = "Palmyra",  mean_pct_predators = 65, pct_other = 35) %>%
  add_row(exp_id = "Northern Line Islands", region = "Jarvis",   mean_pct_predators = 74, pct_other = 26) %>%
  add_row(exp_id = "Northern Line Islands", region = "Baker",    mean_pct_predators = 46, pct_other = 54) %>%
  add_row(exp_id = "Northern Line Islands", region = "Howland",  mean_pct_predators = 54, pct_other = 46) %>%
  add_row(exp_id = "NWHI",                  region = "NWHI",     mean_pct_predators = 54, pct_other = 46) %>%
  # Clean & bound
  mutate(
    exp_id_clean = str_remove(exp_id, "([_\\-\\s]?\\d{4})$"),
    mean_pct_predators = pmax(0, pmin(100, mean_pct_predators)),
    pct_other          = 100 - mean_pct_predators,
    label              = paste(exp_id_clean, region, sep = " — ")
  ) %>%
  distinct(label, .keep_all = TRUE)  # avoid accidental duplicates

# ---- order by predator share (highest at top) ---------------------------
label_levels <- data %>%
  arrange(desc(mean_pct_predators)) %>%
  pull(label)

# ---- long format for plotting ------------------------------------------
plot_df <- data %>%
  select(label, mean_pct_predators, pct_other) %>%
  pivot_longer(
    c(mean_pct_predators, pct_other),
    names_to  = "trophic_group",
    values_to = "percent"
  ) %>%
  mutate(
    trophic_group = recode(trophic_group,
                           "mean_pct_predators" = "Top predators & sharks",
                           "pct_other"          = "Other fishes"),
    label = factor(label, levels = label_levels),
    is_png = if_else(str_detect(as.character(label), "PNG|Papua New Guinea"), TRUE, FALSE)
  )

pred_labels <- plot_df %>%
  filter(trophic_group == "Top predators & sharks") %>%
  mutate(p_label = scales::percent(percent/100, accuracy = 1))

# ---- plot ---------------------------------------------------------------
ggplot(plot_df, aes(x = label, y = percent/100, fill = trophic_group)) +
  geom_col(width = 0.72, color = "white", linewidth = 0.3) +
  geom_text(data = pred_labels,
            aes(label = p_label),
  ...
  scale_y_continuous(labels = scales::label_percent(accuracy = 1),
                     expand = expansion(mult = c(0, 0.02))) +
  scale_fill_manual(
    values = c("Top predators & sharks" = "#1b9e77",
               "Other fishes"            = "#d9d9d9"),
    guide = guide_legend(reverse = TRUE, title = NULL)
  ) +
  labs(
    title    = "Predator & Shark Biomass Share by Region",
    subtitle = "Bars sorted from highest to lowest predator share",
    x        = NULL, y = "Share of total fish biomass",
    caption  = "BLT/UVC means across 10 m and 20 m strata; PNG vs reference sites"
  ) +
  theme_minimal(base_size = 12) +
  theme(
    panel.grid.minor   = element_blank(),
...
data <- png_summ %>%
  ...
  mutate(
    exp_id_clean = str_remove(exp_id, "([_\\-\\s]?\\d{4})$"),
    exp_id_clean = recode(exp_id_clean,
                          "SLB"  = "Solomon Islands",
                          "PLW"  = "Palau",
                          "NWHI" = "North Western Hawaiian Islands"),
    mean_pct_predators = pmax(0, pmin(100, mean_pct_predators)),
    pct_other          = 100 - mean_pct_predators,
    label              = if_else(region != "NWHI", paste(exp_id_clean, region, sep = " — "),exp_id_clean),
    is_png             = str_detect(exp_id_clean, regex("PNG|Papua New Guinea", ignore_case = TRUE))
  ) %>%
  distinct(label, .keep_all = TRUE)

# --- order labels by predator % (highest → lowest) -----------------------
label_levels <- data %>%
  arrange(desc(mean_pct_predators)) %>%
  pull(label)
...
plot_df <- data %>%
  select(label, mean_pct_predators, pct_other, is_png) %>%
  pivot_longer(
    c(mean_pct_predators, pct_other),
    names_to  = "trophic_group",
    values_to = "percent"
...
ggplot(plot_df, aes(x = label, y = percent/100, fill = trophic_group)) +
  ...
  scale_y_continuous(labels = scales::label_percent(accuracy = 1),
                     expand = expansion(mult = c(0, 0.02))) +
  scale_fill_manual(
    values = c("Top predators & sharks" = "#d95f02",  # New predator color
               "Other fishes"            = "#d9d9d9"),
    guide = guide_legend(reverse = TRUE, title = NULL)
  ) +
  labs(
    title    = "Predator & Shark Biomass Share by Region",
    subtitle = "PNG regions bolded",
    x        = NULL, y = "Share of total fish biomass",
    caption = "Reference values — North Western Hawaiian Islands from Friedlander & DeMartini (2002); 
Northern Line Islands from Sandin et al. (2008); all other values from Pristine Seas expeditions."
  ) +
  theme_minimal(base_size = 12) +
  theme(
    axis.text.y = element_text(
      face = ifelse(levels(plot_df$label) %in% data$label[data$is_png], "bold", "plain")
    ),
    panel.grid.minor   = element_blank(),
    panel.grid.major.y = element_blank(),
    legend.position    = "top",
...
ggsave(file.path(exp_path, "figures/uvs_fish_biomass_predator_share_by_region.pdf"), 
       width = 18, height = 12)
```
