...
p <- biomass_by_region_and_trophic |> 
  ...
  geom_col(aes(text = paste0(
    ...
    "Avg. Abundance: ", round(avg_biomass_m2, 2), " ind/m²"
  )))+
...
biomass_by_station_and_trophic |> 
  filter(!is.na(trophic_group)) |> 
  ggplot() +
...
fig <- plot_ly(
  data = biomass_by_station_and_trophic |> filter(!is.na(trophic_group)),
  x = ~region,
  y = ~frac_biomass,
  color = ~trophic_group,
  ...
  text = ~paste0(
    "Region: ", region, "<br>",
    "Trophic group: ", trophic_group, "<br>",
    "Station: ", ps_station_id, "<br>",
    "Biomass fraction: ", round(frac_biomass, 3))
)

fig <- fig %>% layout(
  title = list(
    text = "Proportion of Biomass by Trophic Group and Region",
    font = list(size = 18)
  ),
...
# 1. NMDS
nmds_result <- metaMDS(trophic_matrix, distance = "bray", k = 2, trymax = 100)
...
ggplot(nmds_coords, aes(x = NMDS1, y = NMDS2, color = depth_strata)) +
  ...
  labs(
    title = "NMDS of Trophic Composition with Vector Fit",
